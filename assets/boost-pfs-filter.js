...
general: {
  ...
  limit: 20, // Fix price using Ajax #180299 
  /* Optional */
...
function buildPrice(data) {
  ...
  if (firstVariant) {
    price_min = firstVariant.price;
    compare_at_price_min = firstVariant.compare_at_price;
  }  
  var html = '';
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
    ...
    if (onSale) {
      ...
      html += '<span class="boost-pfs-filter-product-item-sale-price">' + Utils.formatMoney(price_min) + '</span>';
      html += '<s>' + Utils.formatMoney(compare_at_price_min) + '</s> ';
    } else {
      if (priceVaries) {
        html += '<span class="boost-pfs-filter-product-item-price-from-text">' + (boostPFSThemeConfig.label_basic.from) + ' ' + '</span>';
      }
      html += '<span class="boost-pfs-filter-product-item-regular-price">' + Utils.formatMoney(price_min) + '</span>';
    }
    html += '</p>';
  }
  return html;
...
ProductList.prototype.afterRender = function(data) {
  ...
  for (var i = 0; i < data.length; i++) {
      var requestReg = formRequest(data[i].handle);
      requestReg.then((res_success) => {
        var res = JSON.parse(res_success);
        jQ('.boost-pfs-filter-product-item[data-id="'+ res.id +'"]').find('.boost-pfs-filter-product-item-price').html(res.price);
      }).catch((res_error) => {
